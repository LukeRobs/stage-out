// ==UserScript==
// @name         TOs Packed - SeaTalk Hourly Report · Recife 04
// @namespace    spx-express
// @version      1.0
// @updateURL    https://raw.githubusercontent.com/LukeRobs/stage-out/main/tos_packed_report_recife04.user.js
// @downloadURL  https://raw.githubusercontent.com/LukeRobs/stage-out/main/tos_packed_report_recife04.user.js
// @description  Captura screenshot da aba Report de TOs Packed e envia ao SeaTalk a cada hora cheia
// @author       SPX Express
// @match        https://stage-out.onrender.com/tos_packed_recife04.html
...
(function () {
  ...
  async function fetchStats() {
    ...
    list.forEach(to => {
      ...
      const wait = to.complete_time ? nowSec - to.complete_time : 0;
      if (wait > 21600) farol.crit++;
      else if (wait > 14400) farol.urgente++;
      else if (wait > 7200)  farol.atencao++;
      else farol.ok++;
    });
  ...
  function buildReportText(s) {
    ...
    return [
      `Report - TOs Packed · Recife 04 (${data})`,
      `Hora: ${hora}`,
      ``,
      ...
      ``,
      `Link para acompanhamento: https://stage-out.onrender.com/tos_packed_recife04.html`,
    ].join('\n');
  }
  ...
  async function captureReportTab() {
    ...
    if (!btn) { console.warn('[TOs Packed Report] aba Report não encontrada'); return null; }
    btn.click();
    await sleep(3500);
  ...
  function postReport(image, text) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        ...
        data:    JSON.stringify({ tab: 'tos_packed', image, text, station_id: STATION_ID }),
        timeout: 60000,
        onload: r => {
          console.log('[TOs Packed Report] seatalk-report:', r.status, r.responseText);
  ...
  async function sendReport() {
    ...
    try {
      ...
      console.log('[TOs Packed Report] ✅ Report enviado com sucesso!');
    } catch (e) {
      setBadge('❌ Erro no report', '#ef4444');
      console.error('[TOs Packed Report] Erro:', e.message);
    } finally {
  ...
  console.log('[TOs Packed Report] ✅ v1.0 Recife04 — Bot API direto, a cada hora cheia (:00)');
})();
