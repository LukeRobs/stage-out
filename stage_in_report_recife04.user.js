// ==UserScript==
// @name         Stage IN - SeaTalk Hourly Report · Recife 04
// @namespace    spx-express
// @version      1.0
// @updateURL    https://raw.githubusercontent.com/LukeRobs/stage-out/main/stage_in_report_recife04.user.js
// @downloadURL  https://raw.githubusercontent.com/LukeRobs/stage-out/main/stage_in_report_recife04.user.js
// @description  Captura screenshot do dashboard Stage IN e envia ao SeaTalk a cada hora cheia
// @author       SPX Express
// @match        https://stage-out.onrender.com/stage_in_recife04.html
...
(function () {
  ...
  async function fetchReportData() {
    ...
    const resp = await fetch(`${SERVER}/api/report-data?station=${STATION_ID}`);
  ...
  async function fetchVolumosoText() {
    ...
    return [
      ...
      `MIN SPP: ${minSpp}`,
      ``,
      `Link para acompanhamento: https://stage-out.onrender.com/stage_in_recife04.html`,
    ].join('\n');
  }
  ...
  async function fetchTodasText() {
    ...
    return [
      ...
      `  >150       → ${b4} ruas`,
      ``,
      `Link: https://stage-out.onrender.com/stage_in_recife04.html`,
    ].join('\n');
  }
  ...
  function postReport(tab, image, text) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        ...
        data:    JSON.stringify({ tab, image, text, station_id: STATION_ID }),
        timeout: 60000,
        onload: r => {
  ...
  console.log('[Stage IN Report] ✅ v1.0 Recife04 — Bot API direto, a cada hora cheia (:00)');
})();
